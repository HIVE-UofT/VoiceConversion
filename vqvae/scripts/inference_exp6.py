"""
VQ-UNet Experiment 6 — Inference

Converts pre-surgery audio to post-surgery using:
1. WavLM feature extraction (frozen)
2. U-Net encoder → VQ bottleneck (strips quality, preserves structure via skips)
3. Injection of average post-surgery quality vector (modulates skips via FiLM)
4. Decoding back to WavLM features
5. HiFi-GAN vocoding to audio

Usage:
    python scripts/inference_exp6.py
    python scripts/inference_exp6.py --input_dir /path/to/wavs --output_dir /path/to/converted
"""
...
from model.vqvae_unet import VQUNetWavLM
...
def main():
    parser = argparse.ArgumentParser(description="VQ-UNet Exp6 — Inference")
    ...
    parser.add_argument('--output_dir', type=str,
                        default=os.path.join(os.path.dirname(__file__), '..', 'converted_exp6'))
    parser.add_argument('--checkpoint', type=str,
                        default=os.path.join(os.path.dirname(__file__), '..', 'checkpoints_exp6', 'best_vqunet_wavlm.pth'))

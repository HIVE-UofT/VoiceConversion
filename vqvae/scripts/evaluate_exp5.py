"""
Evaluate VQVAE Exp5 voice conversion quality.

Same metrics as all other methods for fair comparison:
  - MCD to target: converted vs real post-surgery (lower = better)
  - Content preservation MCD: source vs converted (lower = better)
  - F0 Correlation: pitch tracking (higher = better)
  - Baseline MCD: real pre vs real post (reference)

Usage:
    python scripts/evaluate_exp5.py
    python scripts/evaluate_exp5.py --converted_dir /path/to/converted --skip_f0
"""
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate VQVAE Exp5 conversion quality")
    ...
    parser.add_argument('--method_name', type=str, default='VQVAE-WavLM (Exp5)')
    parser.add_argument('--skip_f0', action='store_true')

...
class DLAVCModel(nn.Module):
    ...
    def __init__(self, feat_dim=1024, code_dim=64, num_codes=32, num_heads=4,
                 ...
                 commitment_weight=0.25, ema_decay=0.99, entropy_weight=0.5,
                 dropout=0.1, content_noise_std=0.1, use_vq=True):
        super().__init__()
        ...
        # Content bottleneck (+ optional VQ)
    ...
    def forward(self, hidden_states, target_features):
        ...
        if self.use_vq:
            ...
            content_q = content_z
            vq_loss = torch.zeros((), device=content_z.device)
            perplexity = torch.zeros((), device=content_z.device)
    ...
    def encode_content(self, hidden_states):
        """Extract quantized (or continuous if use_vq=False) content from hidden states."""

...
def main():
    ...
    model = DLAVCModel(
        ...
        content_noise_std=CONTENT_NOISE, use_vq=False,
    ).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        for audio_pre, audio_post in paired_loader:
            ...
            if epoch >= WARMUP_EPOCHS:
                ...
                re_cq_a2b = model.content_proj(re_a2b)
                re_b2a, _ = model.unet_encoder(x_b2a)
                re_cq_b2a = model.content_proj(re_b2a)
                # Without VQ, compare raw continuous content codes

...
class UNetDecoder(nn.Module):
    ...
    def forward(self, content, quality, skips):
        ...
        # v2: remove F.instance_norm on skip connections. Instance-normalising
        # strips per-channel magnitude / scale info from the raw skip features,
        # which the decoder needs for accurate feature reconstruction at this
        # data scale. UNet-VC's skips are raw and preserve everything; we now
        # match that, letting FiLM still modulate from quality but on un-normed
        # skips.
        h = self._match(self.up2(h), skip2)
        h = torch.cat([h, self.film2(skip2, quality)], dim=1)
        h = self.dec2(h)

        h = self._match(self.up1(h), skip1)
        h = torch.cat([h, self.film1(skip1, quality)], dim=1)
        h = self.dec1(h)
        return h
...
class DLAVCModel(nn.Module):
    ...
    def __init__(self, feat_dim=1024, code_dim=64, num_codes=32, num_heads=4,
                 ...
                 use_residual_output=True, wavlm_layer_idx=5,
                 alpha_init=0.1):
        super().__init__()

"""
DLA-VC v3 — Multi-Surgery Train Split with ECAPA Speaker Loss + KD Teacher

Multi-surgery variant of dla_vc_v3: trains a single DLA-VC model on the
combined training pools of Tonsill + Fess + Sept. Per-surgery held-out test
patients (5 each, 15 total) are excluded from train and val; the remaining
patients are split per surgery (~15% val, rest train) so each surgery
contributes to both splits.

KD teacher is the multi-surgery UNet-VC-ECAPA model
(unet_vc_ecapa_multi/results_multi_v2/best_model.pt).

Usage:
    python scripts/train_split.py
    python scripts/train_split.py --surgeries Tonsill,Fess,Sept --output results_multi_v3
"""
...
TEACHER_CKPT = '/lustre06/project/6086959/sepharfi/VoiceConversion/Experiments/unet_vc_ecapa_multi/results_multi_v2/best_model.pt'
...
def main():
    ...
    for surg in surgeries:
        ...
        val_pids_s = set(shuffled[:n_val_pids])
        train_pids_s = set(shuffled[n_val_pids:])
        ...
        for pid in all_pids:
            for pre, post in patient_pairs[pid]:
                ...
                if pid in train_pids_s:
                    train_idx.append(idx)
                elif pid in val_pids_s:
                    val_idx.append(idx)
                n_files_surg += 1

        per_surgery_summary[surg] = {
            'patients_total': len(all_pids),
            'train_patients': sorted(train_pids_s),
            'val_patients': sorted(val_pids_s),
        ...
        print(f"  train_pids ({len(train_pids_s)}): {sorted(train_pids_s)}")
        print(f"  val_pids   ({len(val_pids_s)}): {sorted(val_pids_s)}")
    ...
    def _name_for(i):
        return f"{surgery_of_file[i].lower()}_{pid_of_file[i]}_{i:05d}"
    ...
    def evaluate_patients(patient_indices, tag):
        ...
        for i in patient_indices:
            ...
            nm = _name_for(i)
            out_path = os.path.join(conv_dir, nm + '.wav')
            ...
            results.append({'name': nm, 'surgery': surgery_of_file[i],
                            'pid': pid_of_file[i],
                            'sim_post': sp, 'sim_pre': sr_, 'baseline': bl})
            print(f"  [{tag}/{surgery_of_file[i]}] {nm}: conv->post={sp:.3f}  "
                  f"baseline={bl:.3f}  delta={sp-bl:+.3f}")
        return results
    ...
    print(f"\n{'='*70}")
    print(f"  DLA-VC Multi — {','.join(surgeries)} — TRAIN SUMMARY")
    print(f"{'='*70}")
    ...
    with open(os.path.join(args.output, 'results.json'), 'w') as f:
        json.dump({'method': 'DLA-VC-Multi', 'surgeries': surgeries,
                   'train': results_train}, f, indent=2)

"""
LinearVC — Multi-Surgery Evaluation

Same as run_eval.py but the ridge regression is trained on ALL four surgery types:
...
def main():
    ...
    for pid in sorted(test_pre):
        ...
        out_path  = os.path.join(OUT_DIR, f"{pid}_linearvc_multisurg.wav")
    ...
    print_ecapa_summary("LinearVC (MultiSurg)", pids, sims_conv, sims_base)
    ...
    print_ecapa_summary("LinearVC (MultiSurg) [TRAIN SET]", tr_pids, tr_sims_conv, tr_sims_base)

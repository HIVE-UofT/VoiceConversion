"""
UNet-VC-SPK — Test Set Evaluation with Fine-Tuned HiFi-GAN

The model (SpeakerConditionedUNet using WavLM mean-pool speaker embeddings via FiLM)
converts pre-surgery features toward post-surgery style.

1. Loads best_model.pt from results_tonsill/.
...
Usage:
    ...
    python scripts/run_eval.py --checkpoint ../results_tonsill/best_model.pt
"""
...
CKPT     = os.path.join(os.path.dirname(__file__), '..', 'results_tonsill', 'best_model.pt')
...
def main():
    ...
    cfg  = ckpt.get('config', {})
    model = SpeakerConditionedUNet(
        feat_dim=cfg.get('feat_dim', 1024),
        spk_dim=cfg.get('spk_dim', 1024),
    ...
    model.eval()
    print(f'[UNet-VC-SPK] Loaded: epoch={ckpt.get("epoch","?")}  '
    ...
    for pid in sorted(test_pre):
        ...
        out_path  = os.path.join(OUT_DIR, f'{pid}_unetvcspk.wav')

        # WavLM features and utterance-level speaker embedding (mean-pooled WavLM)
        features = knn_vc.get_features(str(pre_path))     # (T, 1024)
        spk_emb  = features.mean(dim=0).unsqueeze(0)       # (1, 1024)
    ...
    print_ecapa_summary('UNet-VC-SPK', pids, sims_conv, sims_base)
    ...
    for pid in sorted(train_pre):
        ...
        spk_emb  = features.mean(dim=0).unsqueeze(0)
        with torch.no_grad():
            converted = model(features.t().unsqueeze(0), spk_emb.to(device)).squeeze(0).t()
        out_wav = knn_vc.vocode(converted[None]).cpu().squeeze()
    ...
    print_ecapa_summary("UNet-VC-SPK [TRAIN SET]", tr_pids, tr_sims_conv, tr_sims_base)

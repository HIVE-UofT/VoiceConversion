"""
VQVAE Experiment 6 — VQ-UNet (WavLM Feature Space)

Hybrid architecture combining U-Net skip connections with VQ bottleneck
and FiLM-conditioned quality modulation. Motivated by Exp5's poor speaker
similarity (0.394 vs 0.661 baseline) — the pure VQVAE bottleneck destroyed
too much information.

Key differences from Exp5:
  - U-Net encoder with skip connections preserves fine-grained detail
  - FiLM layers modulate skip connections based on quality vector
  - VQ bottleneck only at the deepest level (content abstraction)
...
2. Train VQ-UNet to disentangle content (VQ codes) from quality (FiLM modulation)
...
from model.vqvae_unet import VQUNetWavLM, DomainClassifier1D, gradient_reversal
...
CHECKPOINT_DIR = os.path.join(os.path.dirname(__file__), '..', 'checkpoints_exp6')
PLOT_DIR = os.path.join(os.path.dirname(__file__), '..', 'plots_exp6')

# Model
FEAT_DIM = 1024
CODE_DIM = 64            # VQ bottleneck dimension
NUM_CODES = 32           # 32 codes per head
NUM_HEADS = 4            # 4 heads × 16-dim each = 64-dim
QUALITY_DIM = 64         # quality vector dimension (drives FiLM layers)
COMMITMENT_WEIGHT = 0.25
EMA_DECAY = 0.99
ENTROPY_WEIGHT = 0.5     # push against codebook collapse
DROPOUT = 0.15
CONTENT_NOISE = 0.1      # less noise than Exp5 — skip connections help reconstruction
...
WARMUP_EPOCHS = 30       # recon + VQ only
...
# ──────────────────────────────────────────────
# Feature extraction + caching (reuse from Exp5)
# ──────────────────────────────────────────────
...
class WavLMSegmentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return feat.t(), torch.tensor(self.label, dtype=torch.float32)
...
def train():
    ...
    print(f"VQ-UNet parameters: {model.count_parameters():,}")
    ...
    for epoch in range(EPOCHS):
        ...
        for feat_pre, lab_pre, feat_post, lab_post in pbar:
            ...
            if epoch >= WARMUP_EPOCHS:
                with torch.no_grad():
                    enc_out, _ = model.unet_encoder(feat)
                    content_z_all = model.content_proj(enc_out)
            ...
            # ═══ Step 2: Train VQ-UNet + quality classifier ═══
            opt_model.zero_grad()
            opt_qual.zero_grad()
            ...
            if epoch >= WARMUP_EPOCHS:
                ...
                # Encode both domains
                enc_out_pre, skips_pre = model.unet_encoder(feat_pre)
                content_z_pre = model.content_proj(enc_out_pre)
                content_q_pre, _, _ = model.vq(content_z_pre)
                quality_pre = model.quality_encoder(feat_pre)

                enc_out_post, skips_post = model.unet_encoder(feat_post)
                content_z_post = model.content_proj(enc_out_post)
                content_q_post, _, _ = model.vq(content_z_post)
                quality_post = model.quality_encoder(feat_post)

                # Cross-reconstruct: content from A, quality from B, skips from A
                # (skips come from source since they carry structural detail)
                ...
                # Re-encode cross-reconstructed outputs
                re_enc_a2b, _ = model.unet_encoder(cross_pre2post)
                re_content_q_a2b, _, _ = model.vq(model.content_proj(re_enc_a2b))
                re_quality_a2b = model.quality_encoder(cross_pre2post)

                re_enc_b2a, _ = model.unet_encoder(cross_post2pre)
                re_content_q_b2a, _, _ = model.vq(model.content_proj(re_enc_b2a))
        ...
        if avg_val < best_val_loss:
            ...
            }, os.path.join(CHECKPOINT_DIR, 'best_vqunet_wavlm.pth'))
        ...
        if (epoch + 1) % 50 == 0:
            torch.save({
                'epoch': epoch,
                'model': model.state_dict(),
            }, os.path.join(CHECKPOINT_DIR, f'vqunet_wavlm_epoch{epoch+1}.pth'))
...
def compute_avg_quality(model, pre_data, post_data, device, segment_len):
    ...
    avg_pre = torch.cat(qualities_pre, dim=0).mean(dim=0)
...
def plot_training_curves(history, epoch):
    ...
    plt.suptitle(f'VQ-UNet Exp 6 (WavLM + FiLM) — Epoch {epoch}', fontsize=14)
    plt.tight_layout()
